...
void Scene::Draw2D()
{
	...
	for (int s = 0; s < sunNum; s++) {
		...
		//色変更用
		SHADER.m_spriteShader.DrawTex(&sunTex, 0, 0, &rect, &color[sun[s].colorGroup]);
		//第2、第3引数は0固定　行列で座標指定する					//描画しようとしている太陽が所属するグループの色
	}
}
...
void Scene::Init()
{
	...
	for (int s = 0; s < SUN_MAX; s++) {
		...
		sun[s].colorGroup = (ColorGroup)(rand() % GroupMax);
	}
	//太陽の初期数
	sunNum = 1;
...
void Scene::ImGuiUpdate()
{
	...
	if (ImGui::Begin("Debug Window"))		//Beginで作成(失敗はfalseが返る)
	{
		...
		ImGui::Text(u8"色変更");
		//色変更
		ImGui::ColorEdit4("GroupA", &color[GroupA].x);
		ImGui::ColorEdit4("GroupB", &color[GroupB].x);
		ImGui::ColorEdit4("GroupC", &color[GroupC].x);
		ImGui::ColorEdit4("GroupD", &color[GroupD].x);
		ImGui::ColorEdit4("GroupE", &color[GroupE].x);
